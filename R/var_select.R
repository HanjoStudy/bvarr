#' Information criteria and FPE for different frequentist VAR(p)
#'
#' Function adaptation from original functional called \link[VARSelect]{vars} from author Bernhard Pfaff.
...
#'
#' data(Yraw)
#' VARselect(Yraw, lag.max = 5, type = "const")
#'
#' @export
#'
var_select<- function (y, lag.max = 10, type = c("const", "trend", "both", "none"),
                       season = NULL, exogen = NULL, output = c("long", "wide"))
{
  y <- as.matrix(y)
  if (any(is.na(y)))
    stop("\nNAs in y.\n")
  ...
  if(output == "wide"){
   criteria %>%
      data.frame %>%
      purrr::set_names(paste0("lag_", c(1:ncol(.)))) %>%
      tibble::rownames_to_column(., var = "criteria") %>%
      tidyr::gather(., lag, value, -criteria) %>%
      tibble::tbl_df
  }
